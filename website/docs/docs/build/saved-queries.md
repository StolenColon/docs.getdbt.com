...
- Make sure you have the [environment variable](/docs/use-dbt-semantic-layer/exports#set-environment-variable) enabled. 
- Then, use the [`--resource-type` flag](/reference/global-configs/resource-type) and run the command `dbt build --resource-type saved_query`.
